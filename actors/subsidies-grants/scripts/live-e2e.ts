 ...
 * 実gBizINFO APIに対して runSubsidiesGrants をprefill相当入力で流し、
 * 「開始から最初の結果まで30秒以内」（marketing.md §6/§11-4）を計測する。課金はモック。
 *
 *   pnpm --filter @jp-opendata/actor-subsidies-grants exec esbuild scripts/live-e2e.ts \
...
import { runSubsidiesGrants } from '../src/run.js';
...
const summary = await runSubsidiesGrants(
  // prefill相当: 株式会社日立製作所（補助金5件・実証済み。docs/research/gbizinfo-subsidy.md）
