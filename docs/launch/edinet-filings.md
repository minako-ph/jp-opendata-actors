...
| 4 | input prefill実機確認 | **未・ブロック** | Actor main（Phase 1本体: 実行ループ・FR-C7上限・課金発火）未実装＋EDINET_API_KEY未設定のため実行不可。実装後に「Startだけで30秒以内に結果」を実機確認し、prefillの日付を公開日直近に更新すること |
...
1. Actor main実装（Phase 1: 日ループ・FR-C7/C8・billing・N-4集計・財務値CSV(type=5)パース）
2. EDINET_API_KEYで実応答fixture採取→golden差し替え（decisions.md記録済みTODO）
3. `apify push`（手動。CIに含めない）
